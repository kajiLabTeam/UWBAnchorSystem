package net.harutiro.uwbanchorsystemminio.feature.serial.repository

import android.content.Context
import android.util.Log
import net.harutiro.uwbanchorsystemminio.feature.serial.Entity.UWBResult
import net.harutiro.uwbanchorsystemminio.feature.serial.api.SerialApi
import net.harutiro.uwbanchorsystemminio.feature.utils.DateUtils
...
class SerialRepository {
    ...
    fun message2BlinkPacket(message: String):UWBResult? {
        ...
        // raw から 16進数 8文字分取得
        val uciHeader: ByteArray = raw.substring(0, 8)
            .chunked(2)
            .map { it.toInt(16).toByte() }
            .toByteArray()

        val uciPayload: ByteArray = raw.substring(8)
            .chunked(2)
            .map { it.toInt(16).toByte() }
            .toByteArray()
    ...
    fun extractSeqCnt(byteArray: ByteArray): Long {
        return (((byteArray[3].toInt() and 0xFF) shl 24) or
                ((byteArray[2].toInt() and 0xFF) shl 16) or
                ((byteArray[1].toInt() and 0xFF) shl 8) or
                (byteArray[0].toInt() and 0xFF)).toLong()
    }

    fun extractNlos(byteArray: ByteArray): Int = byteArray[28].toInt() and 0xFF

    fun extractDistance(byteArray: ByteArray): Int =
        ((byteArray[30].toInt() and 0xFF) shl 8) or (byteArray[29].toInt() and 0xFF)

    fun extractAzimuth(byteArray: ByteArray): Int =
        ((byteArray[32].toInt() and 0xFF) shl 8) or (byteArray[31].toInt() and 0xFF)

    fun extractAzimuthFom(byteArray: ByteArray): Int = byteArray[33].toInt() and 0xFF

    fun extractElevation(byteArray: ByteArray): Int =
        ((byteArray[35].toInt() and 0xFF) shl 8) or (byteArray[34].toInt() and 0xFF)

    fun extractElevationFom(byteArray: ByteArray): Int = byteArray[36].toInt() and 0xFF

    fun extractDestAzimuth(byteArray: ByteArray): Int =
        ((byteArray[38].toInt() and 0xFF) shl 8) or (byteArray[37].toInt() and 0xFF)

    fun extractDestElevation(byteArray: ByteArray): Int =
        ((byteArray[41].toInt() and 0xFF) shl 8) or (byteArray[40].toInt() and 0xFF)

    fun extractPdoa1(byteArray: ByteArray): Int =
        ((byteArray[71].toInt() and 0xFF) shl 8) or (byteArray[70].toInt() and 0xFF)

    fun extractPdoa2(byteArray: ByteArray): Int =
        ((byteArray[77].toInt() and 0xFF) shl 8) or (byteArray[76].toInt() and 0xFF)

    fun extractRssi(byteArray: ByteArray): Int = byteArray[44].toInt() and 0xFF

    fun convertQFormatToFloat(qIn: Int, nInts: Int, nFracs: Int, roundOf: Int = 2): Double {
